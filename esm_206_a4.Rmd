...
ggplot(lobster_ysite, aes(x = year, y = total_count),
  ...
  geom_dl(aes(label = site, color = MPA), method = list("last.points", cex = .65, hjust = -.17))
...
```{r}
ggplot(lobster_ysite, aes(x = year, y = total_count)) +
  geom_col(aes(fill = MPA), show.legend = FALSE) +
  facet_wrap(~site) +
  theme_bw() +
  labs(title = "Annual Lobster Abundance by Site, 2012 - 2018",
...
```{r}
lobster_mean_size18 <- lobster_tidy %>% 
  filter(year %in% 2018) %>% 
  group_by(site) %>% 
  summarize(total = mean(size_mm))
```



```{r}
lobster_mean_size12 <- lobster_tidy %>% 
  filter(year %in% 2012) %>% 
  group_by(site) %>% 
  summarize(total = mean(size_mm))
```
...
```{r}
ggplot(data = lobster_mean_size18, aes(x = site, y = total, color = "black", fill = site), show.legend = FALSE) +
  geom_col(show.legend = FALSE) +
  labs(title = "Mean Lobster size 2018")

ggplot(data = lobster_mean_size12, aes(x = site, y = total, color = "black", fill = site), show.legend = FALSE) +
  geom_col(show.legend = FALSE) +
  labs(title = "Mean Lobster size 2012")
